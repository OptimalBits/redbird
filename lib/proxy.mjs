...
export function Redbird(opts) {
  if (!(this instanceof Redbird)) {
    return new Redbird(opts);
  }
...
Redbird.prototype.setupLetsencrypt = function (log, opts) {
...
Redbird.prototype.setupHttpsProxy = function (proxy, websocketsUpgrade, log, sslOpts) {
  const _this = this;
  let https;
...
Redbird.prototype.addResolver = function (resolver) {
...
Redbird.prototype.removeResolver = function (resolver) {
  if (this.opts.cluster && cluster.isMaster) return this;
...
Redbird.buildTarget = function (target, opts) {
  opts = opts || {};
 ...
 */
Redbird.prototype.register = function (src, target, opts) {
  if (this.opts.cluster && cluster.isMaster) return this;
  ...
  target = Redbird.buildTarget(target, opts);
...
Redbird.prototype.updateCertificates = function (domain, email, production, renewWithin, renew) {
  const _this = this;
...
Redbird.prototype.unregister = function (src, target) {
  if (this.opts.cluster && cluster.isMaster) return this;
...
Redbird.prototype._defaultResolver = function (host, url) {
...
Redbird.prototype._defaultResolver.priority = 0;
 ...
 * @returns {*}
 */
Redbird.prototype.resolve = function (host, url, req) {
  let resolvedValue;
  const promiseArray = [];
  ...
  return Promise.all(promiseArray)
    .then(function (resolverResults) {
      for (let i = 0; i < resolverResults.length; i++) {
        ...
        if (route && (route = Redbird.buildRoute(route))) {
...
Redbird.buildRoute = function (route) {
  ...
  if (_.isString(route)) {
    routeObject.urls = [Redbird.buildTarget(route)];
    routeObject.path = '/';
  } else {
    ...
    routeObject.urls = (_.isArray(route.url) ? route.url : [route.url]).map(function (url) {
      return Redbird.buildTarget(url, route.opts || {});
    });
...
Redbird.prototype.close = function () {
  try {
    return Promise.all(
