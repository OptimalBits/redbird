...
function containerPort(dolphin, containerId) {
  return dolphin.containers.inspect(containerId).then(function (container) {
    if (process.platform === "darwin") {
      const port = container.NetworkSettings.Ports[Object.keys(container.NetworkSettings.Ports)[0]]["HostPort"];
      const netNames = Object.keys(container.NetworkSettings.Networks);
      if (netNames.length === 1) {
        const ip = container.NetworkSettings.Ports[Object.keys(container.NetworkSettings.Ports)[0]]["HostIp"];
        if (port && ip) {
          return 'http://' + ip + ':' + port;
        }
      } else {
