...
function ETCDModule(redbird, options) {
  ...
  this.etcd_dir =
    typeof options.path !== 'undefined' ? options.path : 'redbird';
  ...
  function createWatcher() {
    ...
    _this.watcher = _this.etcd.watcher(_this.etcd_dir, null, {
      recursive: true
    });
    ...
    _this.watcher.on('change', function(body, headers) {
      if (body.node.key && body.node.value && !IsJsonString(body.node.value)) {
        ...
        if (typeof config.docker !== 'undefined') {
          ...
          _this.redbird.register(
            removeEtcDir(body.node.key),
            config.hosts,
            config
          );
        }
