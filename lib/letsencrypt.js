  ...
  http
    .createServer(function(req, res) {
      ...
      if (isForbiddenPath) {
        logger &&
          logger.info(
            'Forbidden request on LetsEncrypt port %s: %s',
            port,
            filename
          );
        res.writeHead(403);
      ...
      logger &&
        logger.info('LetsEncrypt CA trying to validate challenge %s', filename);
    ...
    if (cert) {
      if (renew) {
        ...
        return le.renew(opts, cert).catch(function(err) {
          logger &&
            logger.error(err, 'Error renewing certificates for ', domain);
        });
      } else {
