    ...
    if(opts.ssl){
      ...
      if(!this.certs[src.hostname]){
        if(opts.ssl.key || opts.ssl.cert || opts.ssl.ca){
          ...
          letsencrypt.getCertificates(src.hostname, opts.ssl.letsencrypt, this.log).then(function(certs){
            //
            // TODO: Set interval for automatic renewing of certificates.
            //
            if(certs){
              _this.certs[src.hostname] = createCredentialContext(certs.key, certs.cert, certs.ca);
            }
          }, function(err){
