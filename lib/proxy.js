...
ReverseProxy.prototype.setupHttpProxy = function (proxy, websocketsUpgrade, log, opts) {
  ...
  var server = this.server = httpServerModule.createServer(function (req, res) {
    var src = _this._getSource(req);
    _this._getTarget(src, req, res).bind(_this).then(function (target) {
      if (target){
...
ReverseProxy.prototype._getTarget = function (src, req, res) {
  ...
  return this.resolve(src, url, req).bind(this).then(function (route) {
    ...
    this.log && this.log.info('Proxying %s to %s', src + url, path.posix.join(target.host, req.url));
...
function getCertData(source, unbundle) {
  ...
  if (source) {
    if (_.isArray(source)) {
      var sources = source;
      return _.flatten(_.map(sources, function (_source) {
