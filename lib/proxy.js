...
function ReverseProxy(opts) {
  ...
  if (opts.cluster && cluster.isMaster) {
    ...
    if (opts.body) {
      ...
      self.bodyChange = opts.body.change;
      var parserName = opts.body.parserName;
      if (opts.body.parser && typeof opts.body.parser === 'function'){
        opts.body.parser(bodyParser);
      }
      if (parserName && typeof parserName === 'string'){
        self.bodyParser = self.bodyParser[parserName]();
      }
      if (opts.body.parserOptions && typeof opts.body.parserOptions === 'object'){
        self.bodyParser = self.bodyParser[parserName](opts.body.parserOptions);
      }
    }
    //
    // Routing table.
    ...
    proxy.on('proxyReq', function (proxyReq, req, res) {
      ...
      if ( req.method.toUppercase !== "GET" && req.body && self.bodyChange) {
        // Add req.body logic here if needed....
        // Make any needed parameter changes
        // Remove body-parser body object from the request
        var body = self.bodyChange(proxyReq, req.body);
...
var proxyWeb = function(req, res, src, proxy, log, opts) {
  var _this = this;
...
ReverseProxy.prototype.setupHttpProxy = function (proxy, websocketsUpgrade, log, opts) {
  ...
  var server = this.server = httpServerModule.createServer(function (req, res) {
    var src = _this._getSource(req);
    if (req.method.toUppercase !== 'GET' && _this.bodyParser) {
      _this.bodyParser(req, res, function (error) {
        proxyWeb.apply(_this, [req, res, src, proxy, log, opts]);
      });
    } else {
      proxyWeb.apply(_this, [req,res, src, proxy, log, opts]);
    }
...
ReverseProxy.prototype.updateCertificates = function (domain, email, production, renewWithin, renew) {
  ...
  return letsencrypt.getCertificates(domain, email, production, renew, this.log).then(function (certs) {
    if (certs) {
      ...
      var renewCertificate = function renewCertificate() {
