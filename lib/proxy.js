  ...
  if (opts.cluster && cluster.isMaster) {
    ...
    if (opts.letsencrypt) {
      ...
      var challengeResolver = function (host, url) {
        if (/^\/.well-known\/acme_challenge/.test(url)) {
          return 'http://127.0.0.1:' + letsencryptPort + '/' + host;
        }
      }
      challengeResolver.priority = 9999;
      opts.resolvers.push(challengeResolver);
    }
    ...
    var server = this.server = http.createServer(function (req, res) {
      ...
      if (target) {
        if (shouldRedirectToHttps(certs, src, target)) {
  ...
  if (opts) {
    if (opts.ssl) {
      ...
      if (!this.certs[src.hostname]) {
        ...
        } else if (opts.ssl.letsencrypt) {
          this.log && this.log.info('Getting Lets Encrypt certificates for %s', src.hostname);
          this.updateCertificates(src.hostname, opts.ssl.letsencrypt);
        } else {
...
ReverseProxy.prototype.updateCertificates = function (domain, email) {
  var _this = this;
  return letsencrypt.getCertificates(domain, email, this.log).then(function (certs) {
    if (certs) {
      _this.certs[src.hostname] = createCredentialContext(certs.privkey, certs.cert, certs.chain);

      // TODO: make renewal timeout configurable
      var renewTime = (certs.expiresAt - Date.now()) - ONE_MONTH;
      setTimeout(function () {
        _this.log & _this.log.info('Renewing letscrypt certificates for %s', domain);
        _this.updateCertificates(domain, email);
      }, renewTime > 0 ? renewTime : 0);
    }
  }, function (err) {
