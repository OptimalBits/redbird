...
ReverseProxy.prototype.setupHttpProxy = function (proxy, websocketsUpgrade, log, opts) {
  ...
  var server = this.server = httpServerModule.createServer(function (req, res) {
    ...
    if (opts.whitelistedIps) {
      if (typeof opts.whitelistedIps !== 'array') throw Error('Option whitelistedIps needs to be a array');
      var addr = proxyaddr(req, false);
      if (!opts.whitelistedIps.includes(addr)) {
        res.writeHead(403);
        res.end();
        return;
      }
